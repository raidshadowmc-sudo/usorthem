...
class Player(db.Model):
    ...
    def get_leaderboard(cls, sort_by='experience', limit=50, offset=0):
        ...
        try:
            ...
            elif sort_by == 'karma': # Added karma sorting
                return cls.query.order_by(cls.karma.desc()).offset(offset).limit(limit).all()
            else:
                return cls.query.order_by(cls.experience.desc()).offset(offset).limit(limit).all()
    ...
    def _get_cached_statistics(cls):
        ...
        stats = db.session.query(
            ...
            func.sum(cls.karma).label('total_karma'), # Added karma sum
            ...
            func.avg(cls.karma).label('average_karma') # Added karma average
        ).first()
        ...
        return {
            ...
            'total_karma': int(stats.total_karma) if stats and stats.total_karma else 0, # Added karma
            ...
            'average_karma': round(stats.average_karma) if stats and stats.average_karma else 0, # Added karma
            'top_player': top_player,
            'richest_player': richest_player,
            'most_reputable_player': most_reputable_player,
            'most_karma_player': most_karma_player # Added karma
        }
    ...
    def add_player(cls, nickname, kills=0, final_kills=0, deaths=0, final_deaths=0, beds_broken=0,
                   ...
                   emerald_collected=0, items_purchased=0, coins=0, reputation=0, karma=0): # Added karma
...
class ASCENDData(db.Model):
    """ASCEND evaluation data for players"""
    __tablename__ = 'ascend_data'

    id = db.Column(db.Integer, primary_key=True)
    player_id = db.Column(db.Integer, db.ForeignKey('players.id'), nullable=False, unique=True)

    # Individual skill tiers
    pvp_tier = db.Column(db.String(5), default='D')
    clutching_tier = db.Column(db.String(5), default='D')
    block_placement_tier = db.Column(db.String(5), default='D')
    gamesense_tier = db.Column(db.String(5), default='D')
    overall_tier = db.Column(db.String(5), default='D')

    # Numerical scores (0-100)
    pvp_score = db.Column(db.Integer, default=25)
    clutching_score = db.Column(db.Integer, default=25)
    block_placement_score = db.Column(db.Integer, default=25)
    gamesense_score = db.Column(db.Integer, default=25)

    # Evaluation metadata
    comment = db.Column(db.Text, default='')
    evaluator_name = db.Column(db.String(100), default='Elite Squad')
    evaluator_id = db.Column(db.Integer, db.ForeignKey('players.id'), nullable=True)
    previous_tier = db.Column(db.String(5), nullable=True)
...
class CursorTheme(db.Model):
    ...
    name = db.Column(db.String(50), nullable=False, unique=True)
    display_name = db.Column(db.String(100), nullable=False)
    cursor_url = db.Column(db.String(255), nullable=True)
    is_active = db.Column(db.Boolean, default=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    ...
    def create_default_cursors(cls):
        ...
        default_cursors = [
            {
                'name': 'default',
                'display_name': 'Стандартный курсор',
                'cursor_url': None
            }
        ]
...
class Clan(db.Model):
    ...
    # Relationships
    leader = db.relationship('Player', backref='led_clans')
...
class ClanMember(db.Model):
    ...
    def __repr__(self):
        return f'<ClanMember {self.clan_id}:{self.player_id}>'
...
class TournamentParticipant(db.Model):
    ...
    def __repr__(self):
        return f'<TournamentParticipant {self.tournament_id}:{self.player_id}>'